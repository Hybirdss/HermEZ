...
### Provider 값 (공식 `chat_parser` choices 기준 — `hermes_cli/main.py`)

`model.provider` 에 들어갈 수 있는 전체 값:

| 값 | 설명 | 필요한 크레덴셜 |
|---|------|----------------|
| `auto` | 있는 키로 자동 선택 (기본값) | — |
| `openrouter` | OpenRouter (다수 모델 프록시) | `OPENROUTER_API_KEY` 또는 `OPENAI_API_KEY` |
| `anthropic` | Anthropic 직결 | `ANTHROPIC_API_KEY` 또는 `ANTHROPIC_TOKEN` (OAuth 금지) |
| `openai-codex` | ChatGPT 계정 OAuth | `hermes login --provider openai-codex` |
| `copilot` / `copilot-acp` | GitHub Copilot (CLI / ACP IDE) | `GITHUB_TOKEN` |
| `gemini` | Google AI Studio 직결 | `GOOGLE_API_KEY` 또는 `GEMINI_API_KEY` |
| `huggingface` | HuggingFace Inference | `HF_TOKEN` |
| `zai` | z.ai / ZhipuAI GLM | `GLM_API_KEY` (또는 alias `ZAI_API_KEY` / `Z_AI_API_KEY`) |
| `kimi-coding` / `kimi-coding-cn` | Kimi / Moonshot (글로벌/중국) | `KIMI_API_KEY` / `KIMI_CN_API_KEY` |
| `minimax` / `minimax-cn` | MiniMax (글로벌/중국) | `MINIMAX_API_KEY` / `MINIMAX_CN_API_KEY` |
| `nous` | Nous Portal OAuth | `hermes login --provider nous` (+ 선택적 `NOUS_API_KEY`) |
| `kilocode` | KiloCode gateway | `KILOCODE_API_KEY` |
| `xiaomi` | Xiaomi MiMo | `XIAOMI_API_KEY` |
| `arcee` | Arcee AI Trinity | `ARCEEAI_API_KEY` |
| `custom` | 로컬 LMStudio/Ollama/OpenAI-compat | `model.base_url` + (선택) `OPENAI_API_KEY` |

추가 env var (doctor 가 인식):
- `OPENAI_BASE_URL` — custom 엔드포인트 감지용
- `DEEPSEEK_API_KEY` — DeepSeek 직결
- `DASHSCOPE_API_KEY` — Alibaba DashScope/Qwen
- `AI_GATEWAY_API_KEY` — Vercel AI Gateway
- `OPENCODE_ZEN_API_KEY` / `OPENCODE_GO_API_KEY` — OpenCode 게이트웨이

> **`openai` 는 공식 provider 목록에 없음.** OpenAI 모델 쓰려면 OpenRouter 경유 (provider: `openrouter` + `OPENAI_API_KEY`) 또는 ChatGPT OAuth (provider: `openai-codex`).
...
hermes doctor --fix
```
공식 doctor 가 감지한 문제 중 자동 수정 가능한 것들을 알아서 고친다. 이걸로 해결되면 루프 종료.
효과 없으면 아래 루프로 진행.
...
새 이슈 템플릿은 STEP 0 환경 정보 + 에러 + 시도한 조치 + **`hermes dump` 출력** (공식 지원 덤프 명령) 을 자동으로 포함.

덤프 수집:
```bash
hermes dump                # 키는 "set/not set" 표시만
# 또는 키 앞뒤 4자리 보여주기 (공유 전 확인용):
hermes dump --show-keys
```

브라우저 오픈 (규칙 패턴): `https://github.com/NousResearch/hermes-agent/issues/new?title=<URL-encoded>&body=<URL-encoded>`.
...
| `hermes chat -q "질문"` | 단발 비대화식 (스크립트용) |
| `hermes chat --image <경로> -q "설명"` | 이미지 첨부 단발 |
| `hermes chat --resume <session_id>` | 특정 세션 이어가기 |
| `hermes chat --continue [이름]` | 이름 또는 가장 최근 세션 이어가기 |
| `hermes chat --worktree` | 격리된 git worktree 에서 실행 (병렬 에이전트) |
| `hermes chat --checkpoints` | 파괴적 파일 작업 전 스냅샷 (`/rollback` 으로 복원) |
| `hermes chat --yolo` | 위험 명령 승인 스킵 (주의) |
| `hermes chat --max-turns 30` | 턴당 툴 호출 횟수 제한 |
| `hermes version` | 버전 + Python/SDK/업데이트 여부 |
| `hermes status` | 상태 점검 (간단) |
| `hermes status --all` | 상세 (redacted 공유 가능) |
| `hermes status --deep` | 딥 체크 (오래 걸림) |
| `hermes doctor` | 상세 진단 |
| `hermes doctor --fix` | **자동 수정 시도** (자가치유에 강력) |
| `hermes dump` | 지원 요청용 환경 요약 출력 |
| `hermes dump --show-keys` | 키 앞뒤 4자리 포함 |
| `hermes logs` | 로그 뷰어 |
| `hermes update` | 업데이트 |
| `hermes uninstall` | 제거 |
