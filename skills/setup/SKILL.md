...
### 키 네이밍 — 공식 `hermes_cli/config.py` `set_config_value` 기준

`hermes config set <KEY> <value>` 는 KEY 의 패턴으로 저장 위치 자동 선택 (소스 line 3294):

```python
if key.upper() in api_keys or key.upper().endswith(('_API_KEY', '_TOKEN')):
    save_env_value(key.upper(), value)   # → ~/.hermes/.env
else:
    user_config[dotted_path] = value      # → ~/.hermes/config.yaml
```

**명시적 `.env` 대상 목록** (그 외 `_API_KEY` / `_TOKEN` 끝만 자동 감지):

`OPENROUTER_API_KEY`, `OPENAI_API_KEY`, `ANTHROPIC_API_KEY`, `VOICE_TOOLS_OPENAI_KEY`, `EXA_API_KEY`, `PARALLEL_API_KEY`, `FIRECRAWL_API_KEY`, `FIRECRAWL_API_URL`, `FIRECRAWL_GATEWAY_URL`, `TOOL_GATEWAY_DOMAIN`, `TOOL_GATEWAY_SCHEME`, `TOOL_GATEWAY_USER_TOKEN`, `TAVILY_API_KEY`, `BROWSERBASE_API_KEY`, `BROWSERBASE_PROJECT_ID`, `BROWSER_USE_API_KEY`, `FAL_KEY`, `TELEGRAM_BOT_TOKEN`, `DISCORD_BOT_TOKEN`, `TERMINAL_SSH_HOST`, `TERMINAL_SSH_USER`, `TERMINAL_SSH_KEY`, `SUDO_PASSWORD`, `SLACK_BOT_TOKEN`, `SLACK_APP_TOKEN`, `GITHUB_TOKEN`, `HONCHO_API_KEY`, `WANDB_API_KEY`, `TINKER_API_KEY`

### ⚠️ `config set` 이 잘못 저장하는 env var (직접 `.env` 편집 필수)

아래 env var 는 Hermes 런타임이 **환경변수로 읽지만**, 이름이 `_API_KEY` / `_TOKEN` 으로 안 끝나고 명시 목록에도 없음 → `hermes config set` 이 YAML 로 잘못 보냄 → **동작 안 함**:

| 이름 | 용도 | 저장 방법 |
|------|-----|---------|
| `TELEGRAM_ALLOWED_USERS` | Telegram 허용 유저 ID | `.env` 직접 편집 |
| `DISCORD_ALLOWED_USERS` | Discord 허용 유저 ID | `.env` 직접 편집 |
| `OPENAI_BASE_URL` | OpenAI endpoint override | `.env` 직접 편집 |
| `GEMINI_BASE_URL` / `GLM_BASE_URL` / `KIMI_BASE_URL` / `MINIMAX_BASE_URL` / `ARCEE_BASE_URL` | Provider endpoint override | `.env` 직접 편집 |

**직접 편집 방법 (append 안전):**
```bash
# POSIX
cat >> ~/.hermes/.env <<'HERMES_ENV_EOF'
TELEGRAM_ALLOWED_USERS="123456789"
DISCORD_ALLOWED_USERS="987654321012345678"
HERMES_ENV_EOF
chmod 600 ~/.hermes/.env

# PowerShell
Add-Content -Path "$env:LOCALAPPDATA\hermes\.env" -Value 'TELEGRAM_ALLOWED_USERS="123456789"'
```

> Claude 가 `hermes config set TELEGRAM_ALLOWED_USERS ...` 실행 후 `hermes config get telegram_allowed_users` 하면 값이 YAML 에 저장돼있지만, gateway 는 env var 로만 읽어서 작동 안 함. 발견 시 **즉시 `.env` 로 이동**.

### YAML (`config.yaml`) 에 저장되는 값 — 점 표기법

- `model.provider`, `model.default`, `model.base_url`, `model.context_length`
- `terminal.backend`, `agent.max_turns`, `display.skin`, 등
- `_API_KEY` / `_TOKEN` 접미 없고 명시 목록 밖 → 자동으로 YAML 로 감
...
> - **권장 — ChatGPT OAuth** (선택 2): 구독 있으면 크레딧 지급 + 토큰 자동 갱신으로 더 유리
> - **API 키 그대로 쓰려면 `custom` provider + `model.base_url` (YAML 점 표기):**
>   ```bash
...
>   ```
> ⚠️ `OPENAI_BASE_URL` 같은 BASE_URL env var 는 `hermes config set` 이 YAML 로 잘못 저장함. `model.base_url` (점 표기법) 써야 YAML 의 올바른 경로에 들어감. 또는 `.env` 직접 편집.
...
AskUserQuestion 2번 (별개 freeform 입력):
1. "봇 토큰 붙여넣어 주세요"
2. "Telegram 사용자 ID (숫자)"

**저장 — 두 값이 다른 방식:**
```bash
# 토큰: _TOKEN 끝나므로 config set 자동으로 .env 로 감
 hermes config set TELEGRAM_BOT_TOKEN "[토큰]"

# ALLOWED_USERS: config set 이 YAML 에 잘못 저장. 직접 .env 에 추가:
 cat >> ~/.hermes/.env <<'HERMES_ENV_EOF'
...
> `hermes config set TELEGRAM_ALLOWED_USERS` 쓰면 gateway 가 못 읽음 — config.py 의 자동 분류 로직 때문. 반드시 `.env` 직접 append.
...
**저장 — Telegram 과 동일 규칙 (토큰은 config set, ALLOWED_USERS 는 .env 직접):**
```bash
...
| `hermes config get <path>` / `set <key> "<값>"` | 조회 / 저장 (⚠ `_ALLOWED_USERS` · `_BASE_URL` 은 `.env` 직접 편집) |
...
| Telegram / Discord 봇이 메시지 못 받음 | `hermes config get telegram_allowed_users` 값이 있으면 YAML 오저장 버그. `~/.hermes/.env` 에 `TELEGRAM_ALLOWED_USERS="..."` 로 이동 후 gateway 재시작 |
| `OPENAI_BASE_URL` 안 먹힘 | `config set` 이 YAML 로 보냄. `model.base_url` (점 표기) 쓰거나 `.env` 직접 편집 |
| BASE_URL 계열 일반 | 위와 동일 원리. `.env` 직접 편집 또는 `model.base_url` 사용 |

---

## 🧪 설치 후 실전 체크리스트

사용자에게 아래 5개 한 번씩 해보라고 권유:

```
□ 1. 터미널에서 `hermes` 치고 엔터 → 인터랙티브 CLI 뜸 (/exit 로 종료)
□ 2. `hermes chat -q "안녕"` → 짧은 응답 받음
□ 3. `hermes doctor` → 모든 체크 ✓
□ 4. 메신저 설정한 경우: 폰 봇에게 "안녕" 전송 → 답장 받음
□ 5. `hermes dashboard` → http://localhost:9119 열림 (Ctrl+C 로 종료)
```

하나라도 실패하면 `hermes dump` 출력 공유 + 지원 채널 섹션 참조.

---

## 🔄 재실행 / 롤백 / 업데이트 / Uninstall

### 이미 설치된 사용자가 이 SKILL.md 재실행 시

STEP 0 가 `~/.hermes` 존재 감지하면 AskUserQuestion:
```
질문: "기존 설치 감지됨. 어떻게 할까요?"
선택지:
  - 🔄 재설정 (키·모델만 다시) — STEP 3 부터
  - 🔼 업데이트 (버전 올리기) — hermes update
  - 🆕 완전 초기화 (백업 후 재설치)
  - ❌ 취소
```
...
# 필요시 세션/스킬 복원
cp -r ~/.hermes.bak-*/sessions ~/.hermes/ 2>/dev/null
cp -r ~/.hermes.bak-*/skills ~/.hermes/ 2>/dev/null
```

### 업데이트

```bash
hermes update        # config / 세션 / 스킬 보존됨
```
...
```bash
hermes uninstall     # 공식 제거 (TTY 필수 — 확인 프롬프트)
rm -rf ~/.hermes     # 데이터까지 완전 삭제 (주의, 복원 불가)
```

### 백업 / 이식

```bash
hermes backup                          # ~/.hermes.zip
hermes profile export <name> <path>    # 특정 프로필만
hermes import <path>                   # 복원
```
...
1. **공식 FAQ**: https://hermes-agent.nousresearch.com/docs/reference/faq
2. **공식 Discord**: https://discord.gg/NousResearch (가장 빠른 응답)
3. **공식 이슈 트래커**: https://github.com/NousResearch/hermes-agent/issues
4. **HermEZ 이슈 (이 스킬 자체 문제)**: https://github.com/Hybirdss/HermEZ/issues
5. **Awesome Hermes (커뮤니티 스킬)**: https://github.com/0xNyk/awesome-hermes-agent

새 이슈 올릴 때:
- `hermes dump` 출력 (키 자동 redacted)
- 에러 전체 메시지
- 재현 단계

> ⚠️ `hermes dump --show-keys` 는 Discord DM / 비공개 채널에서만. public issue 에 금지.
